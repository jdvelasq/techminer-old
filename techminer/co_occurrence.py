...
def summary_co_occurrence(
    x, column, by=None, keywords=None, min_frequency=1, min_cited_by=0
):
    ...
    >>> summary_co_occurrence(df, column='Author_Keywords', by='Authors')
      Author_Keywords Authors  Num_Documents  Cited_by      ID
    0               a       A              2         1  [0, 1]
    1               a       B              1         1     [1]
    2               b       A              1         1     [1]
    3               b       B              2         3  [1, 2]
    ...
    >>> summary_co_occurrence(df, 'Authors', 'Author_Keywords', keywords=keywords)
      Authors Author_Keywords  Num_Documents  Cited_by      ID
    0       B               a              1         1     [1]
    1       B               b              2         3  [1, 2]
...
def co_occurrence(
    ...
    >>> co_occurrence(df, column='Authors', by='Author_Keywords')
      Authors Author_Keywords  Num_Documents      ID
    0       A               a              2  [0, 1]
    1       B               b              2  [1, 2]
    2       B               c              2  [3, 4]
    3       A               b              1     [1]
    ...
    >>> co_occurrence(df, column='Author_Keywords', by='Authors', as_matrix=True)
       a  b  c  d
    A  2  1  1  0
    B  1  2  2  1
    C  0  0  1  0
    ...
    >>> co_occurrence(df, column='Author_Keywords', by='Authors', as_matrix=True, min_value=2)
       a  b  c
    A  2  1  1
    B  1  2  2

    >>> co_occurrence(df, column='Author_Keywords', by='Authors', as_matrix=True, min_value=5)
       a  b  c
    A  2  1  1
    B  1  2  2
    ...
    >>> keywords = keywords.compile()
    >>> co_occurrence(df, column='Author_Keywords', by='Authors', as_matrix=True, keywords=keywords)
       c  d
    A  1  0
    B  2  1
...
def summary_occurrence(x, column, keywords=None):
    ...
    >>> summary_occurrence(df, column='Authors')
       Authors Authors_  Num_Documents  Cited_by            ID
    0        A        A              4         7  [0, 1, 2, 4]
    1        A        B              2         6        [2, 4]
    2        A        C              1         4           [4]
    3        B        A              2         6        [2, 4]
...
def occurrence(
    x, column, as_matrix=False, normalization=None, min_value=0, keywords=None
):
    ...
    >>> occurrence(df, column='Authors')
       Authors Authors_  Num_Documents            ID
    0        A        A              4  [0, 1, 2, 4]
    1        B        B              4  [2, 3, 4, 6]
    2        A        B              2        [2, 4]
    3        B        A              2        [2, 4]
...
def __TAB0__(x):
    ...
    controls = [
        ...
        # 4
        {
            ...
            "widget": widgets.Dropdown(
                options=["1"], ensure_option=True, layout=Layout(width=WIDGET_WIDTH),
            ),
        },
        # 4
        {
    ...
    def server(**kwargs):
        ...
        matrix = co_occurrence(
            x, column=term, by=by, as_matrix=False, min_value=0, keywords=None,
        )
        #
...
def __TAB1__(x):
    ...
    def server(**kwargs):
        ...
        #
        matrix = occurrence(
            x, column=term, as_matrix=False, min_value=0, keywords=None,
        )
        #
        options = matrix["Num_Documents"].unique().tolist()
        options = sorted(options)
        ...
        with output:
            ...
            if plot_type == "Network plot":
                display(
                    occurrence_map(matrix, layout=layout, cmap=cmap, figsize=(14, 8.5))
                )
